...
void cut_above_f(LSSimpleFrameState *sfs, int arg) {
    lprintf("f() state:\n");
    ...
    f_rsp[- 1] = sfs->rip;
    f_rsp[- 2] = (uint64_t)arg;
    f_rsp[- 3] = (uint64_t)gprime;
    f_rsp[- 4] = 0LL;
    f_rsp[- 5] = (uint64_t)gprime_starter;
    f_rsp[- 6] = sfs->rbp;
    f_rsp[- 7] = sfs->rbx;
    f_rsp[- 8] = sfs->r12;
    f_rsp[- 9] = sfs->r13;
    ...
    f_rsp[-11] = sfs->r15;
    f_rsp[-12] = 0LL;
    f_rsp[-13] = (uint64_t)ls_swap_in;

    mainStack.sp = &f_rsp[-13];
}

const int fact = 10;

void spy(void *arg) {
    lprintf("Hi! I will replace g()\n");
    lprintf("My return address is %p\n", __builtin_return_address(0));
    ...
    lprintf("Start replacement. I'll let it calculate the factorial of %d.\n", fact);
    cut_above_f(&sfs, fact);
